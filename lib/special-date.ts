export const startRegisDate = new Date("February 3, 2024 03:00:00").getTime();

export const startComingSoonAnnouncementDate = new Date(
  "February 13, 2024 16:00:00"
).getTime();

export const startAnnouncementDate = new Date(
  "February 20, 2024 03:00:00"
).getTime();

export const confirmationDate = new Date(
  "February 23, 2024 17:00:00"
).getTime();

// 1 Februari 08:00 WIB
export const startVotingDate = new Date("February 1, 2024 01:00:00").getTime();

// 7 Februari 21:00 WIB
export const endVotingDate = new Date("February 7, 2024 13:00:00").getTime();

export const mainEventEndDate = new Date("March 2, 2024 13:30:00").getTime();

export const feedbackStartDate = new Date("March 1, 2024 08:00:00").getTime();

export const feedbackEndDate = new Date("March 2, 2024 08:00:00").getTime();
